...
?>
   ...
   <?php
      if(isset($_POST['check']))
      {
          ...
          $query=mysqli_query($con,"select * from  profiles where name like '%{$search}%' ");   // || status like '%{$search}%' ");
         ...
         //     echo  '<tr><td>'.$row['name'].'</td></tr>';
             echo '<h4>'.$row['name'].'</h4>';
         /*  echo'
                <form method=POST action=SingleAlumni.php>
                <button type=submit name="fname">'.$row['name'].'</button>
               <input type="hidden" name="hide" value='.$row['fid'].'/>
               </form>';
*/
            }
          }
   ...
   ?>
    ...
    ?>
  		<div class="team-member">
  			<ul>
  				<li>
  	<?php		echo '<img class="imgcircle" src='.$row['img_src'].' alt=""/>';    ?>
  					<div class="clearfix"></div>
            <form method=POST action=SingleAlumni.php>
  	<?php	
          echo'<button type=submit name="fname">'.$row['name'].'</button>
               <br><label>Assistant Professor</label>
               <input type="hidden" name="hide" value='.$row['fid'].'/>';
     ?>
            </form>
  				</li>
